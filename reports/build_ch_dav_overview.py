...
EVENTS = [
    ("1997-08-12", "processing", "Flux calculation bug fixed: 10 s pre-averaging had suppressed transport by eddies slower than 10 s."),
    ("2002-01-09", "setup", "EC boom found loose and rotated towards south for an unknown time; re-aligned to the west. Pyrradiometer dome inflation broken."),
    ("2005-08-09", "instrument", "Open-path LI-7500 replaces closed-path LI-6262; new data acquisition."),
    ("2006-06-12", "radiation", "CNR1 net radiometer connected; double pyranometer removed. Later found: SW_OUT sign and LW_OUT/SW_OUT channels swapped (corrected in processing)."),
    ("2006-10-31", "site", "Harvest of 25 × 70 m (1750 m²) in the NE part of the footprint."),
    ("2006-12-20", "instrument", "Sonic Gill R2 replaced by R3-50."),
    ("2008-11-19", "soil", "Soil heat flux plate found wrongly connected."),
    ("2012-09-25", "instrument", "Enclosed-path LI-7200 installed."),
    ("2013-11-08", "site", "Thinning inside the tower footprint."),
    ("2014-07-16", "instrument", "ICOS system: Gill HS-50 + LI-7200 on the same boom as the old system (both until 2016-11)."),
    ("2014-10-16", "soil", "Five self-calibrating heat flux plates (5–6 cm) replace the single plate."),
    ("2016-06-15", "instrument", "Heated intake tube fitted to the LI-7200 (ICOS standard)."),
    ("2017-04-12", "radiation", "CNR4 four-component radiometer installed on a south-pointing boom."),
    ("2019-11-18", "network", "ICOS Class 1 labelling; wrong wind-direction configuration of the sonic found and fixed (north offset 270°)."),
    ("2020-09-22", "instrument", "Interim HS-100 while the HS-50 is factory-calibrated (until 2021-02)."),
    ("2021-11-08", "instrument", "LI-7200RS replaces the LI-7200."),
    ("2022-02-09", "instrument", "Sonic transducer found out of place (since unknown); HS-50 sent for repair."),
    ("2023-05-02", "instrument", "Replacement HS-50 installed; ~2° inclination noise noted."),
]
